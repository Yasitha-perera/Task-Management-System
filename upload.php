...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["file"]) && $_FILES["file"]["error"] == 0) {
        ...
        if (file_exists($target_file)) {
            ...
            window.location.href ='index.php'; // Redirect to admin page after successful upload
          </script>";
        } else {
            ...
            if (!in_array($file_type, $allowed_types)) {
                ...
                // Retrieve selected task and user from the form
                ...
                if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
                    ...
                    $sql = "INSERT INTO files (filename, username, task, susername) VALUES ('$filename', '$username', '$selected_task', '$selected_user')";

                    if ($conn->query($sql) === TRUE) {
                        echo "<script type='text/javascript'>
                                alert('File Uploaded Successfully.');
                                window.location.href ='index.php'; // Redirect to index page after successful upload
                              </script>";
                    } else {
